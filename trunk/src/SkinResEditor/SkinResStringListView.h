...
public:
    ...
    virtual void InitResult(HTREEITEM hTreeItem)
    {
        ...
        if (m_hWnd == NULL)
        {
            ...
            m_edit1.Create(m_hWnd, 
                rcDefault, 
                NULL, 
                WS_CHILD | WS_BORDER | ES_AUTOHSCROLL);
            m_edit1.SetFont(GetFont());

            m_edit2.Create(m_hWnd, 
                rcDefault, 
                NULL, 
                WS_POPUP | WS_BORDER | ES_MULTILINE | ES_AUTOVSCROLL | ES_WANTRETURN | WS_VSCROLL,
                WS_EX_TOPMOST);
            m_edit2.SetFont(GetFont());

            GetEditProc(m_edit1) = (WNDPROC)::SetWindowLong(m_edit1, GWL_WNDPROC, (LONG) HookEditMsg );
            GetEditProc(m_edit2) = (WNDPROC)::SetWindowLong(m_edit2, GWL_WNDPROC, (LONG) HookEditMsg );
    ...
    static LRESULT CALLBACK HookEditMsg( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
    {
        ...
        if (uMsg == WM_KILLFOCUS)
            ::SendMessage(GetNotifyWnd(), WM_EDITKILLFOCUS, 0, 0);

        return lResult;
    }

    static HWND& GetNotifyWnd()
    {
        static HWND gs_wndNotify = NULL;
    ...
    {
        static HWND    gs_wndEdit[2] = { 0 };
        static WNDPROC gs_wndEditProc[2] = { 0 };
        
        if (gs_wndEdit[0] == NULL)
            gs_wndEdit[0] = hWnd;

        if ( gs_wndEdit[0] == hWnd )
        {
            return gs_wndEditProc[0];
        }

        gs_wndEdit[1] = hWnd;

        return gs_wndEditProc[1];
    }
    ...
    LRESULT OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
    {
        ...
        for (int sub = 1; sub <= 2; sub++)
        {
            ...
            if (::PtInRect(&rcSubItem, info.pt))
            {
                ...
                CEdit m_edit = sub == 1 ? m_edit1 : m_edit2;
                ...
                if ( sub == 2 )
                {
                    ...
                    if ( rcSubItem.right > rt.right )
                    {
                        int nWidth = rcSubItem.right - rcSubItem.left;

                        rcSubItem.left  = rt.right - nWidth;
                        rcSubItem.right = rt.right;

                    }
                    if ( rcSubItem.bottom > rt.bottom )
                    {
                        int nHeight = rcSubItem.bottom  - rcSubItem.top;

                        rcSubItem.top = rt.bottom - nHeight;
                        rcSubItem.bottom = rt.bottom;
                    }
                }
    ...
    LRESULT OnEditKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
    {
        if (m_neditindex >= 0)
        {
            ...
            TCHAR szNewBuffer[10240] = { 0 };

            CEdit m_edit = m_neditsubitem == 1 ? m_edit1 : m_edit2;

            m_edit.GetWindowText(szNewBuffer, 10240);
                ...
                if (m_neditindex == (GetItemCount() - 1))
                {
                    if (_tcscmp(szNewBuffer, _T("IDS_")))
                    {
                        ...
                        CListViewCtrl::GetItemText(m_neditindex, 2,
                            item.strValue.GetBuffer(10250), 10240);
