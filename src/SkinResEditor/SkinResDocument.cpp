...
BOOL SkinResDocument::NewDocument()
{
    ...
    if (m_pskinconfig != NULL)
        delete m_pskinconfig;

    m_pskinconfig = new skinconfig;
    m_pskinconfig->m_pathSkinResPath.RemoveFileSpec();
    m_pskinconfig->m_pathSkinResPath.RemoveFileSpec();
    m_pskinconfig->m_pathSkinImagePath = m_pskinconfig->m_pathSkinResPath;
    m_pskinconfig->m_pathSkinImagePath.Append(_T("images"));

    m_resImageDoc.m_pskinconfig = m_pskinconfig;
...
BOOL SkinResDocument::OpenDocument(const KSGUI::CString& strFileName)
{
    ...
    path.Append(_T("images"));
    m_pskinconfig->m_pathSkinImagePath = path;

    m_resImageDoc.m_pskinconfig = m_pskinconfig;

    _Module.init_skin(strFileName.Mid(m_pskinconfig->m_pathSkinResPath.m_strPath.GetLength() ), 
        m_pskinconfig);
