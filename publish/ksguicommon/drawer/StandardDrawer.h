...
enum ENUM_CHECKBOX_STATUS
{
    ...
    CBS_NORMAL   = 0x0001,    // 普通状态
    CBS_SELECTED = 0x0002,    // 选中无焦点状态
    CBS_FOCUS    = 0x0003,    // 选中有焦点状态
    CBS_DISABLED = 0x0004,    // 禁止状态

    // make combination with:
    CBS_CHECKED  = 0x0100     // check状态 
};
...
inline void DrawCheckBox( HDC hdc, int x, int y, UINT uStatus )
{
    ...
    if( uStatus & CBS_DISABLED )
    {
        crFrame = 0xBBC8CA;
        crInner = 0xFFFFFF;
        crTick  = 0xBBC8CA;
    }
    else if( uStatus & CBS_NORMAL )
    {
        crFrame = 0x80511C;
        crInner = 0xFDFDFD;
        crTick  = 0x21A121;
    }
    else if( uStatus & CBS_SELECTED )
    {
        crFrame = 0x80511C;
        crInner = 0xFDFDFD;
        crTick  = 0x21A121;
    }
    else if( uStatus & CBS_FOCUS )
    {
        crFrame = 0x80511C;
        crInner = 0xFDFDFD;
        crTick  = 0x21A121;
