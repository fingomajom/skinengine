...
BOOL dasm_get_instruction_size (BYTE * pInst, UINT * pnSize, UINT * pnSizeIfMoved) 
{
    int nSize        = 0 ;  
    int nSizeIfMoved = 0 ;
    ...
    case 0x64:  // FS instruction prefix
        if( dasm_get_instruction_size(pInst+1,&nSize,&nSizeIfMoved) )
        {
            nSize += 1 ;
            nSizeIfMoved += 1 ;
        }
...
BOOL  dasm_move_program (BYTE * pSrc, UINT nSrcSize, BYTE * pDst, UINT * pnDstSize, UINT nDstMax)
{
    UINT    nReadBytes    = 0 ;
    UINT    nWritBytes    = 0 ;
    int		nReadInstSize = 0;
    ...
    while( nReadBytes < nSrcSize )
    {
        ...
        bSuccess = dasm_get_instruction_size (pReadInst, &nReadInstSize, &nWritInstSize) ;
