...
public:
	...
	CQuickserachView m_view;
	CCommandBarCtrl m_CmdBar;

    CAboutDlg m_addrband;

	virtual BOOL PreTranslateMessage(MSG* pMsg)
	{
        CQuickSearchMgr& mgt = CQuickSearchMgr::Instance();

        if ( ::IsWindow(m_addrband) &&
            m_addrband.IsWindowVisible() &&
            m_addrband.IsDialogMessage( pMsg ) )
            return TRUE;

        if ( ::IsWindow(mgt.m_wndQSPopWnd) &&
             mgt.m_wndQSPopWnd.IsWindowVisible() &&
...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
        ...
        m_addrband.Create( m_hWnd );

		CreateSimpleReBar(ATL_SIMPLE_REBAR_NOBORDER_STYLE);
		AddSimpleReBarBand(hWndCmdBar);
        AddSimpleReBarBand(hWndToolBar, NULL, TRUE);
        AddSimpleReBarBand(m_addrband , NULL, TRUE);
