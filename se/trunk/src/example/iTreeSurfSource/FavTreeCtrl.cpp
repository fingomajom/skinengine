...
void CFavTreeCtrl::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ((hItem != NULL) && !ItemHasChildren(hItem))
	{
		...
		{
			CiTreeSurfView* pvw = GetCurrentView(1, theApp._configFile._bFavInNewWindow);
...
void CFavTreeCtrl::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ((hItem != NULL) && GetItemData(hItem)!=-1)
	{
		...
		pGroups = pPopup->GetSubMenu(0);
		 ( theApp._globalSpace._pMainFrame )->MenuLoadGroupList(pGroups);
...
void CFavTreeCtrl::OnRfavDel() 
{
	...
	CString path,szPath;
	if(! ( theApp._globalSpace._pMainFrame )->GetFavoriteFolder(szPath))
		return ;
...
/*	HTREEITEM hItem = GetParentItem(m_hItem);
	...
	//update menu
	 ( theApp._globalSpace._pMainFrame )->m_bFavDirty = TRUE;
...
void CFavTreeCtrl::OnRfavOpenall() 
{
	...
	while(hit!=NULL && (nWins< theApp._configFile._nMaxWins || theApp._configFile._nMaxWins ==0))
	{
		if(GetItemData(hit)!=-1)
		{
			if(nWins!=0)
			{
				...
				if(pos!=NULL)
					GetURL(m_astrFav.GetAt(pos), url);
				 ( theApp._globalSpace._pMainFrame )->m_astrOpenAll.AddTail(url);
				nWins++;
			}
			else
			{
				CiTreeSurfView* pvw = CreateNewView();
				int d = GetItemData(hit);
				if(pvw!=NULL)
				{
					CString url;
	...
	if(nWins >1 && theApp._configFile._nInterval )
		 ( theApp._globalSpace._pMainFrame )->m_nTimerID = ::SetTimer( ( theApp._globalSpace._pMainFrame )->GetSafeHwnd(), 1000, theApp._configFile._nInterval*1000,NULL);
	else if(nWins>1)
		 ( theApp._globalSpace._pMainFrame )->m_nTimerID = ::SetTimer( ( theApp._globalSpace._pMainFrame )->GetSafeHwnd(), 1000, 20,NULL);
	//enable toolbar button
	 ( theApp._globalSpace._pMainFrame )->m_wndToolBar.GetToolBarCtrl().EnableButton(ID_FILE_STOP_OPENALL);
}
...
void CFavTreeCtrl::OnRfavStartFolder() 
{
	...
	if (hItem != NULL )
	{
		...
		CString oldpath =  theApp._configFile._csFavoritePath ;
		if(oldpath == path)
		{
			//clean the startup folder
			 theApp._configFile._csFavoritePath  = "";
		...
		//save the path
		 theApp._configFile._csFavoritePath  = path;
...
void CFavTreeCtrl::OnRfavStart() 
{
	...
	if(!CopyFile(path+strName+".url",  theApp._configFile._csFavoritePath +strName+".url",TRUE))
	{
		if(CopyFile(path+strName+".lnk",  theApp._configFile._csFavoritePath +strName+".lnk",TRUE))
		{
...
void CFavTreeCtrl::OnRfavMostFav() 
{
	...
	if(OpenFile(path+strName+".url", &ofs, OF_EXIST)!=HFILE_ERROR)
	{
		...
		if(nImage == 1)
		{
			...
			theApp._configFile.Set_MostFavor( "1", path+strName+".url" );
		}
		else if (nImage == 4)
		{
			SetItemImage(m_hItem, 1,1);
			theApp._configFile.Set_MostFavor( "0", path+strName+".url" );
		}
		//update menu
		 ( theApp._globalSpace._pMainFrame )->m_bFavDirty = TRUE;
	}
...
BOOL CFavTreeCtrl::GetURL(CString &path, CString &url)
{
	...
	{
		TCHAR buf[2048];
		if( ( theApp._globalSpace._pMainFrame )->ResolveLink(url, buf))
		{
			url = buf;
			path = url;
		}
...
void CFavTreeCtrl::OnRfavMostFolder() 
{
	// TODO: Add your command handler code here
	CString strName;
	strName = _ITREE_SURF_MOST_FAVOR_FILE;
	CString path,szPath;
	if(! ( theApp._globalSpace._pMainFrame )->GetFavoriteFolder(szPath))
		return ;
...
//	if(CreateFile(path+strName, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_HIDDEN, NULL)!=NULL)
	{
		...
		if(nImage == 0)
		{
			...
			theApp._configFile.Set_MostFavor( "1", path+strName );
		}
		else if (nImage == 5)
		{
			SetItemImage(m_hItem, 0,2);
			theApp._configFile.Set_MostFavor( "0", path+strName );
		}
...
void CFavTreeCtrl::OnRfavProperty() 
{
	...
	else
	{
		...
		{
			CString csFavorUrl="";
			theApp._configFile.GetFavorUrl( csFavorUrl, path+strName+".url" );
			...
			{
				if(pos!=NULL)
					m_astrFav.SetAt(pos, csFavorUrl);
				//update menu
				 ( theApp._globalSpace._pMainFrame )->m_bFavDirty = TRUE;
...
void CFavTreeCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	else
	{
		...
		if(hItem != m_hOldDropItem)
		{
			...
			( theApp._globalSpace._pMainFrame )->SetMessageText(url);
		...
		if(!bMoveFolder)
		{
			...
			if(source!=target)
			{
				...
				if(CopyFile(source, target, TRUE))
				{
					...
					//not update menu
					 ( theApp._globalSpace._pMainFrame )->m_bFavDirty = FALSE;
		...
		else
		{
			...
			if(source!=target)
			{
				...
				if(MoveFile(source, target))
				{
					...
					if(source+"\\" ==  theApp._configFile._csFavoritePath )
					{
					...
					//move sub order
					CString str;
					if( ( theApp._globalSpace._pMainFrame )->GetFavoriteFolder(str))
					{
...
void CFavTreeCtrl::MoveSubMenuOrder(LPCSTR source, LPCSTR target)
{
	HKEY hSrcKey;
	HKEY hTarKey;
	CString lpSubKey = theApp._globalSpace._lpstrMenuOrderKey, lpSubKey2;
	lpSubKey += source;
...
//				if(e!=NULL)e->Delete();
			...
			//copy it
			lpSubKey = theApp._globalSpace._lpstrMenuOrderKey;
			lpSubKey += target;
...
void CFavTreeCtrl::OnAddtoGroup(UINT nID)
{
	...
	if(d>=0)
	{
		...
		name = GetItemText(m_hItem);
		 ( theApp._globalSpace._pMainFrame )->AddtoGroup(name, url, nID); 
	}	
...
void CFavTreeCtrl::OnRfavfolderReset() 
{
	...
	lpSubKey = theApp._globalSpace._lpstrMenuOrderKey + lpSubKey;
	if(m_hItem != GetRootItem())
		lpSubKey += strName;
