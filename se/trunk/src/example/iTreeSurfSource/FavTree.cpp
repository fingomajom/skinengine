...
CFavTree::CFavTree()
{
	theApp._configFile.GetDw_ShowHidden( m_dwHidden );
...
BOOL CFavTree::Initialize()
{
	...
	if(m_bFirstInitial)
	{
		m_wndFav.m_bBest = theApp._configFile._bMostFavChecked;
	...
	if(szPath ==  theApp._configFile._csFavoritePath )
	{
...
void CFavTree::Destroy()
{
	...
	m_wndFav.m_FavTreeCtrl.DeleteAllItems();
	m_wndFav.m_FavTreeCtrl.m_astrFav.RemoveAll();
	theApp._configFile._bMostFavChecked = m_wndFav.m_bBest;
	...
	if(h != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			if((wfd.dwFileAttributes & fileattr)==0)
			{
				...
				{
					theApp._configFile.GetFavorUrl( csUrl, strPath2 + str ); 
					CString csMostFav="";
					theApp._configFile.Get_MostFavor( csMostFav, strPath2 + str );
					...
					if(!bBest || (bBest && csMostFav[0]=='1'))
					{
						nPos = nEndPos;
						astrFavorites.Add(str2);
						anFavID.Add(nPos);
						m_wndFav.m_FavTreeCtrl.m_astrFav.AddTail( csUrl );
						++nEndPos;
					}
				}
				else  if(!bBest && str.Right(4) == _T(".lnk"))
				{
					//if(!bBest &&  ( theApp._globalSpace._pMainFrame )->ResolveLink(strPath2 + str, buf))
					{
		...
		WORD type;
		char mff[10];
		if( ( theApp._globalSpace._pMainFrame )->GetFavoriteFolder(strPath))
		{
				...
				for(int i = 0; i<mo.len; i++)
				{
					...
					for(j = 0; j<astrDirs.GetSize() && !found && (type==0x10 || type==0x14 || type==0); j++)
					{
						if(astrDirs[j].CompareNoCase(str)==0)
						{
							//add
							CString csMostFav="\0";
							theApp._configFile.Get_MostFavor( csMostFav, strPath2+astrDirs[j]+"\\" + _ITREE_SURF_MOST_FAVOR_FILE );
...
//							else
							{
								...
								if(strPath2+astrDirs[j]+"\\" ==  theApp._configFile._csFavoritePath )
								{
		...
		for(i = 0; i<astrDirs.GetSize(); i++)
		{
			...
			CString csMostFav="\0";
			theApp._configFile.Get_MostFavor( csMostFav, strPath2+astrDirs[i]+"\\" + _ITREE_SURF_MOST_FAVOR_FILE );
...
//			else
			{
			//	hLastDir = hnode;
				if(strPath2+astrDirs[i]+"\\" ==  theApp._configFile._csFavoritePath )
				{
					m_wndFav.m_FavTreeCtrl.m_hStartFolder = hnode;
					m_wndFav.m_FavTreeCtrl.SetItemState(hnode,TVIS_BOLD,TVIS_BOLD);
				}
