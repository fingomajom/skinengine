		...
		VARIANT* PostData, VARIANT* Headers, BOOL* Cancel)
{
	...
	if(m_bIniDL)
	{
		if( theApp._configFile._pDownload!=NULL && (strURL.Find('?')<0 || strURL.Find(".jsp?")>0))
		{
			try{
			...
			if(IsDownloadable(strURL))
			{
				...
				theApp._configFile._pDownload->QueryInterface(IID_IObjectWithSite, (void**)&pSite);
				if(pSite!=NULL)
				{
	...
	/*		//	VARIANT VarResult;
			...
			//	unsigned int uArgErr=0;
			  ...
			  //
				...
				HRESULT hr = theApp._configFile._pDownload->Invoke(250, IID_NULL,
...
//327
	...
	{
		::GetClientRect( ( theApp._globalSpace._pMainFrame )->m_hWndMDIClient, &rectClient);
		rectClient.top -= 2;
		rectClient.left -= 2;
		rectClient.bottom += 2;
		rectClient.right += 2;
...
BOOL CFixedHtmlView::IsDownloadable(CString &URL)
{
	...
	if(i>0)
	{
		...
		char ch;
		i=0;
		int dllen = theApp._configFile._csDownloadFileExt.GetLength();
		while(i<dllen)
		{
			ch = theApp._configFile._csDownloadFileExt.GetAt(i);
