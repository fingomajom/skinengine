		   ...
		   )
{
     ...
     if (pguidCmdGroup && IsEqualGUID(*pguidCmdGroup, CGID_DocHostCommandHandler))
      {
         if((nCmdID == 40 || nCmdID == 41) && ( theApp._globalSpace._pMainFrame )!=NULL && theApp._configFile._bSilent )//OLECMDID_SHOWSCRIPTERROR
         {
			 if(nCmdID == 41)
			 {
			 ...
			 if(nCmdID == 40)
				( theApp._globalSpace._pMainFrame )->SetMessageText("Script Error!");
			 else if(bActiveX)
				 ( theApp._globalSpace._pMainFrame )->SetMessageText("ActiveX Denied!");
			 else
			 {
...
HRESULT FAR EXPORT  CCustomControlSite::XDocHostUIHandler::GetHostInfo( DOCHOSTUIINFO* pInfo )
{
	...
	try{
	METHOD_PROLOGUE(CCustomControlSite, DocHostUIHandler)
	pInfo->dwFlags = DOCHOSTUIFLAG_ENABLE_FORMS_AUTOCOMPLETE |DOCHOSTUIFLAG_FLAT_SCROLLBAR;// | DOCHOSTUIFLAG_CODEPAGELINKEDFONTS|DOCHOSTUIFLAG_NO3DBORDER;
