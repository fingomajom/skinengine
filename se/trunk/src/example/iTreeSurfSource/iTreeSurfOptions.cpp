// iTreeSurfOptions.cpp : implementation file
//
...
BOOL CDownloadProp::OnInitDialog() 
{
	...
	for(int i=0; i<= theApp._configFile._aDownloadTools.GetUpperBound(); i++)
	{
		m_SelectTools.AddString( theApp._configFile._aDownloadTools[i].DName );
	}
...
BOOL COtherProp::OnInitDialog() 
{
	...
	int nc = ( theApp._globalSpace._pMainFrame )->m_wndAddress->SendMessage(CB_GETCOUNT, 0, 0);
	for(int i=0;i<nc;i++)
	{
		item.iItem = i;
		( theApp._globalSpace._pMainFrame )->m_wndAddress->GetItem(&item);
...
void COtherProp::OnOK() 
{
	...
	( theApp._globalSpace._pMainFrame )->m_wndAddress->ResetContent();

	for(int i=0;i<nc;i++)
	{
		item.iItem = i;
		m_Addresses.GetLBText(i, sz);
		( theApp._globalSpace._pMainFrame )->m_wndAddress->InsertItem(&item);
	}	
	( theApp._globalSpace._pMainFrame )->SaveTypedURL();
