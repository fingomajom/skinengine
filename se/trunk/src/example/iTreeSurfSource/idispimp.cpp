		...
		try
		{
			...
			case DISPID_ADDFAVORITE:
				if(pDispParams->cArgs == 2)
				   ( theApp._globalSpace._pMainFrame )->AddtoFavorite(pDispParams->rgvarg[1].bstrVal, &pDispParams->rgvarg[0]);
				else if(pDispParams->cArgs == 1)
				   ( theApp._globalSpace._pMainFrame )->AddtoFavorite(pDispParams->rgvarg[0].bstrVal, &pDispParams->rgvarg[0]);
				break;
			case DISPID_ADDPROXY:
				if(pDispParams->cArgs == 4)
				{
					...
					if(webproxy > 0)
					{
						...
						CMenu* pMenu = ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(5)->GetSubMenu(2);
						( theApp._globalSpace._pMainFrame )->BuildWebProxyMenu(pMenu, 1); //translate
						pMenu = ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(5)->GetSubMenu(3);
						( theApp._globalSpace._pMainFrame )->BuildWebProxyMenu(pMenu, 0); //web proxy
...
//		str2 = theApp._globalSpace._lpFiles;
	}
