...
{
	m_bDispFile = TRUE;
	m_strFilter = theApp._configFile._csFileTypes;
...
CShellTreeCtrl::~CShellTreeCtrl()
{
	try
	{
		theApp._configFile.Set_FileTypes( m_strFilter );
	}
	catch(...)
	{
	}
...
*
****************************************************************************/
/*BOOL CShellTreeCtrl::GetSelectedFolderPath(CString &szFolderPath)
{
	LPTVITEMDATA	lptvid;  //Long pointer to TreeView item data
	LPSHELLFOLDER	lpsf2=NULL;
	static char		szBuff[MAX_PATH];
...
void CShellTreeCtrl::OnClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if(SHGetPathFromIDList(lptvid->lpifq,szBuff))
	{
		...
		if(OpenFile(szBuff, &ofs, OF_EXIST) != HFILE_ERROR)
		{
			...
			{
				CiTreeSurfView* pvw = GetCurrentView(1, theApp._configFile._bFileInNewWindow);
				if(pvw!=NULL)
				{
