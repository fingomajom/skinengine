					...
					if( id == ID_LINK_POPUP)
					{
						...
						if( pTBar->GetToolBarCtrl().GetButton( iCount,  &tbb ))
						{
							if(IsMenu((HMENU)tbb.dwData))
							{
								...
								SetMenuText( theApp._globalSpace._csaLinks[iCount-1], md);
							//	md->nID = id;
							//	md->nFlags = iMenuStyle;
								HMENU hmenu= CopyMenu((HMENU)tbb.dwData);
								pop.AppendMenu ( iMenuStyle|MF_OWNERDRAW, (UINT)hmenu, (char*)md );//spaces+theApp._globalSpace._csaLinks[iCount-1]
							}
							else
								delete md;
						}
...
//						md->nFlags = iMenuStyle;
					...
					{
						FavSize = ( theApp._globalSpace._pMainFrame )->m_nFavSize;
						iMenuStyle |= MF_POPUP;
						TBBUTTON tbb;
						if( pTBar->GetToolBarCtrl().GetButton( iCount,  &tbb ))
						{
							...
							{
								pTBar->GetButtonInfo ( iCount, id, iStyle, md->m_nBitmap);
								SetMenuText( theApp._globalSpace._csaFavors[iCount], md);
							//	md->nID = id;
							//	md->nFlags = iMenuStyle;
								HMENU hmenu= CopyMenu((HMENU)tbb.dwData);
								pop.AppendMenu ( iMenuStyle|MF_OWNERDRAW, (UINT)hmenu, (char*)md );//spaces+theApp._globalSpace._csaLinks[iCount-1]
					...
					{
						pTBar->GetButtonInfo ( iCount, id, iStyle, md->m_nBitmap);
					//	POSITION pos = ( theApp._globalSpace._pMainFrame )->m_lstrUtilNames.FindIndex(iCount);
						SetMenuText( theApp._configFile._ExternalUtilList.m_UtilList.GetAt(iCount)->m_strUtilName, md);
						pop.AppendMenu ( iMenuStyle|MF_OWNERDRAW, id , (char*)md );
					}
...
//							md->nFlags = iMenuStyle;
							...
							{
							//	BOOL bMax;
							//	( theApp._globalSpace._pMainFrame )->MDIGetActive(&bMax);

								// the font popup is stored in a resource
							//	if (bMax)
							//		pPopup = ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(1)->GetSubMenu(0);
							//	else
									pPopup = ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(0)->GetSubMenu(0);
							}
							else if(id== ID_BMODE_DROPDOWN)
							{
								pPopup = ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(5)->GetSubMenu(9);
							}
							else if(id== ID_TOOLS_SETBG)
							{
								pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(6)->GetSubMenu(20);
							}
							else if(id== ID_GROUP)
							{
								pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(4)->GetSubMenu(0);
							}
							else if(id== ID_OPTIONS_LOADIMAGE)
							{
								pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(5)->GetSubMenu(0);
							}
							else if(id== ID_TOOLS_AUTOSCROLL_USEAUTOSCROLLING)
							{
								pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(6)->GetSubMenu(19);
							}
							...
							{
								pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(6)->GetSubMenu(11);
							}
							else if(id == ID_OPTIONS_USE_PROXY)
							{
								...
								int i=0;
								//CString str, str1;
				/*				POSITION pos1= ( theApp._globalSpace._pMainFrame )->m_aProxySpeed.GetHeadPosition(), pos =  ( theApp._globalSpace._pMainFrame )->m_astrProxy.GetHeadPosition(), pos2 =  ( theApp._globalSpace._pMainFrame )->m_astrProxyName.GetHeadPosition();
								while(pos!=NULL && pos1!=NULL && pos2!=NULL)
								{
									str =  ( theApp._globalSpace._pMainFrame )->m_astrProxyName.GetAt(pos2);
									str = str.Left(50);
									str1.LoadString(IDS_SP_INVALID +  ( theApp._globalSpace._pMainFrame )->m_aProxySpeed.GetAt(pos1));
									str1 = "(" + str1 + ")";
									if(str.Left(1)=="-")
									{
									...
									if(bUseProxy && theApp._configFile._csCurrentProxy ==  ( theApp._globalSpace._pMainFrame )->m_astrProxy.GetAt(pos))
										menu.CheckMenuItem(0x1100 + i, MF_CHECKED | MF_BYCOMMAND );
									 ( theApp._globalSpace._pMainFrame )->m_astrProxy.GetNext(pos);
									 ( theApp._globalSpace._pMainFrame )->m_astrProxyName.GetNext(pos2);
									 ( theApp._globalSpace._pMainFrame )->m_aProxySpeed.GetNext(pos1);
									i++;
								}*/
								CProxyList* pl;
								CMenu submenu;
									...
									if(pl!=NULL && !pl->m_bIsWebProxy)
									{
										...
										for(i=0; i<=pl->m_Proxies.GetUpperBound(); i++)
										{
											...
											else
											{
												...
												if(bUseProxy && theApp._configFile._csCurrentProxy == pl->m_Proxies.ElementAt(i)->m_strProxy)
												{
													if(pcates>1)
							...
							else if(id == ID_OPTIONS_USE_WEBP)
							{
								...
								CChildFrame* tcf = (CChildFrame*)( ( theApp._globalSpace._pMainFrame )->MDIGetActive());
								if (tcf!=NULL && tcf->m_pView!=NULL)
								{
									if(((CiTreeSurfView*)tcf->m_pView)->m_bEnWebProxy)
									{
										...
										if(strCurWebProxy=="" || strCurWebProxy.IsEmpty())
											strCurWebProxy = theApp._configFile._csDefaultWebProxy;
									}
								}

								// the font popup is stored in a resource
								menu.CreatePopupMenu();
							//	POSITION pos =  ( theApp._globalSpace._pMainFrame )->m_astrWebProxy.GetHeadPosition(), pos1 =  ( theApp._globalSpace._pMainFrame )->m_aWebProxySpeed.GetHeadPosition(), pos2 =  ( theApp._globalSpace._pMainFrame )->m_astrWebProxyName.GetHeadPosition();
								/*3.1
								CProxyList* pl = theApp._configFile._aProxyCategories.GetWebProxyList();
								int i=0;
								if(pl!=NULL)
									for(i=0; i<=pl->m_Proxies.GetUpperBound(); i++)
									{
								 ...
								 ( theApp._globalSpace._pMainFrame )->BuildWebProxyMenu(&menu, 2);
								pPopup = &menu;
							}
...
//						md->nFlags = iMenuStyle;
				...
				else if(rbinfo.wID == ID_MENU_BAR)
				{
					...
					{
						//CString str;
						 ( theApp._globalSpace._pMainFrame )->GetMenu()->GetMenuString(iCount, str, MF_BYPOSITION);
						CMenu * pPopup;
						pPopup =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(iCount);
						if(pPopup!=NULL)
						{
							HMENU hmenu = CopyMenu(pPopup->m_hMenu);
	...
	if ( bAtleastOne )
		pop.TrackPopupMenu ( TPM_LEFTALIGN|TPM_TOPALIGN, ptMenu.x, ptMenu.y, ( theApp._globalSpace._pMainFrame ) );

	if(rbinfo.wID == ID_VIEW_FAVS)
		 ( theApp._globalSpace._pMainFrame )->m_nFavSize=FavSize;
...
void CChevBar::OnParentNotify(UINT message, LPARAM lParam) 
{
	...
	if(message == WM_RBUTTONDOWN)
	{
		...
		if(pw!=NULL && (pw->m_hWnd ==   ( theApp._globalSpace._pMainFrame )->m_wndTab.GetSafeHwnd() 
			|| ::IsChild( ( theApp._globalSpace._pMainFrame )->m_wndAddress->GetSafeHwnd(), pw->m_hWnd)))
			return;
		CMenu *pmenu;
		pmenu =  ( theApp._globalSpace._pMainFrame )->GetMenu()->GetSubMenu(2)->GetSubMenu(0);

		//2002-05-30
		CRect crMenu0, crMenu11;
		GetMenuItemRect( pw->m_hWnd, pmenu->m_hMenu, 0, crMenu0 );
		int iMenuWidth = crMenu0.Width();
		GetMenuItemRect( pw->m_hWnd, pmenu->m_hMenu, 11, crMenu11 );
		int iMenuHeight = crMenu11.bottom - crMenu0.top;
		
        int ixborder = GetSystemMetrics(SM_CXSCREEN);
		int iyborder = GetSystemMetrics(SM_CYSCREEN);

		int iX = point.x;
		int iY = point.y;

		bool bCrossBorder=false;
		UINT uFlag=TPM_LEFTBUTTON;

		if( iX + iMenuWidth + 5 >= ixborder )
		{
			iX = ixborder;// - iMenuWidth;
			bCrossBorder = true;
			uFlag |= TPM_RIGHTALIGN;
		}
		else
		{
			uFlag |= TPM_LEFTALIGN;
		...
		if( iY + iMenuHeight + 5 >= iyborder )
		{
			iY = iyborder;// - iMenuHeight;
			bCrossBorder = true;
			uFlag |= TPM_BOTTOMALIGN;
		}
		else
		{
			uFlag |= TPM_TOPALIGN;
		...
		if(pmenu!=NULL)
			if( bCrossBorder )
				pmenu->TrackPopupMenu( uFlag , iX, iY, AfxGetMainWnd() );
			else
