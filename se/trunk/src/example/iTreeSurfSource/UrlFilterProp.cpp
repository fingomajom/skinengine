...
BOOL CUrlFilterProp::OnInitDialog() 
{
	...
	{
		m_cURLList.InsertItem(i, theApp._configFile._csaUrlFilter.GetAt(i));
	}
...
void CUrlFilterProp::OnOK() 
{
	...
	theApp._configFile._csaUrlFilter.RemoveAll();
	int n = m_cURLList.GetItemCount();
	for( int i=0; i<n; i++)
	{
		theApp._configFile._csaUrlFilter.Add(m_cURLList.GetItemText(i,0));
	}
	( theApp._globalSpace._pMainFrame )->m_bUrlFilterDirty = TRUE;
	( theApp._globalSpace._pMainFrame )->InitialUrlFilter();
