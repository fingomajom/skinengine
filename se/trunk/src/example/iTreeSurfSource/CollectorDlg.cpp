...
void CCollectorDlg::SaveCollector()
{
	...
	GetWindowRect(&wrect);
	theApp._configFile.Set_CollectWindowRect( wrect );
	...
	CFile f;
	if(f.Open(((CiTreeSurfApp*)app)->m_strSaveTxtFile, CFile::modeCreate|CFile::modeWrite|CFile::shareDenyNone))
	{
                           ...
                           DROPEFFECT dropEffect, CPoint point)
{
	...
	/*	int si = GlobalSize(hLinkData);
		...
		str2+="\r\n\r\n";
		if( ( theApp._globalSpace._pMainFrame )->pCollectDlg)
		{
			int len =  ( theApp._globalSpace._pMainFrame )->pCollectDlg->m_Editor.GetWindowTextLength( );
			 ( theApp._globalSpace._pMainFrame )->pCollectDlg->m_Editor.SetSel(len, len);
			 ( theApp._globalSpace._pMainFrame )->pCollectDlg->m_Editor.ReplaceSel(str2);
		}
		else
		{
			CFile f;
			if(f.Open(theApp.m_strSaveTxtFile, CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite|CFile::shareDenyNone))
			{
				f.SeekToEnd();
	...
	else
	{
		...
		DragQueryFile(hdrop, 0, theApp._globalSpace._lpFiles, 1024);
		GlobalUnlock(hData);

		CString filename, rfile;
		filename = theApp._globalSpace._lpFiles;
		...
		filename = theApp._configFile._csToolbarPath + filename;
//		if(theApp.m_bNeedCreateDir)
		{
			CreateDirectory(theApp._configFile._csToolbarPath, NULL);
//			theApp.m_bNeedCreateDir = FALSE;
		}
		
		//compare file size
		DWORD sz1, sz2=0;
		HANDLE hfile = INVALID_HANDLE_VALUE;
		hfile = CreateFile( theApp._globalSpace._lpFiles, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		if(!CopyFile( theApp._globalSpace._lpFiles, rfile, FALSE))
		{
