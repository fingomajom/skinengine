...
//	HBITMAP hbmp = ::LoadBitmap(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_COLDTOOLBAR));
	HBITMAP	hbmp = (HBITMAP)LoadImage(NULL, theApp._configFile._csBitmapRoot+"\\coldtool.bmp", IMAGE_BITMAP, 0, 0, 
...
//	bitmap.Detach();
	...
	{
		if( theApp._globalSpace._iaToolbarButtons[i]!=0)
		{
			str.LoadString( theApp._globalSpace._iaToolbarLabels[i]);
			m_avabtn.InsertItem(n, str.Right(str.GetLength()-str.Find('\n')-1), theApp._globalSpace._iaToolbarButtonImgs[i]);
...
void CCustomizeToolbar::OnOK() 
{
	...
	for( int i=0; i<n; i++)
	{
		...
		theApp._configFile.WriteToolBarID( tmp, id );
	}
