...
int CLinkListDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	SetIcon(LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_iTreeSurfYPE)),FALSE);
...
void CLinkListDlg::OnDestroy() 
{
	...
	if(!IsIconic())
	{
		...
		GetWindowRect(&wrect);
		theApp._configFile.Set_LinkWindowRect( wrect );
	}

	CFile::Remove(theApp.m_strLinkListPath+"tmp");
}
