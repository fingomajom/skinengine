...
BOOL COrgGroupDlg::OnInitDialog() 
{
	...
	//load group list
	m_strStartGroup = theApp._configFile._csStartGroup;
...
void COrgGroupDlg::FillUrlList(int index)
{
	...
	filename = theApp._configFile._csGroupPath + filename+".cgp";

	theApp._configFile.GetGroupUrl( m_UrlList, filename );
...
/*	char * valuepair = NULL;
	...
	try{
		...
		if(nsize>0)
		{
			...
			theApp._configFile.GetGroupSection( valuepair, nsize, filename );
...
void COrgGroupDlg::SaveUrlList(int index)
{
	...
	theApp._configFile.Set_GroupUrl( m_UrlList, filename );
...
void COrgGroupDlg::OnNewGroup() 
{
	...
	CString filename;
	filename = theApp._configFile._csGroupPath + str+".cgp";
...
void COrgGroupDlg::OnEndlabeleditGroupList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(MoveFile(filename, theApp._configFile._csGroupPath + str + ".cgp"))
	{
...
void COrgGroupDlg::OnOK() 
{
	...
	theApp._configFile._csStartGroup = m_strStartGroup;
...
void COrgGroupDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bDragging)
	{
		...
		if (pDropWnd == m_pDropWnd && m_nDropIndex>=0 && m_nDropIndex!=m_nLastSelItemID)
		{
			...
			CString filename = m_GroupList.GetItemText(m_nDropIndex, 0);
			theApp._configFile.AddUrlToGroup( filename, name, url );
...
void COrgGroupDlg::OnDblclkUrlList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	( theApp._globalSpace._pMainFrame )->NewChildWindow(1,2, m_UrlList.GetItemText(item,1));
