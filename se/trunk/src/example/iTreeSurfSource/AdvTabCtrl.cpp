...
{
	// TODO: Add your message handler code here and/or call default
	if( theApp._configFile._nAllowDBC > 0 )
	{
...
{
	// TODO: Add your message handler code here and/or call default
	if( theApp._configFile._nAllowMC > 0 )
	{
...
void CAdvTabCtrl::ProcessClick(UINT nFlags, CPoint point, int nClickButton)
{
	try{
		...
		if(TabCtrlItem.lParam != NULL)
		{
			...
			if((nClickButton==0 && theApp._configFile._nAllowDBC==1) 
				|| 
				(nClickButton==1 && theApp._configFile._nAllowMC==1) 
				|| 
				(nClickButton==2 && theApp._configFile._nAllowRC==1)
				)
			{
			...
			else if((nClickButton==0 && theApp._configFile._nAllowDBC==2) 
				|| 
				(nClickButton==1 && theApp._configFile._nAllowMC==2) 
				|| 
				(nClickButton==2 && theApp._configFile._nAllowRC==2)
				)
			{
				//save as
				tcf->m_pView->PostMessage(WM_COMMAND, ID_FILE_SAVEAS, 0);
			}
			else if((nClickButton==0 && theApp._configFile._nAllowDBC==3) 
				|| 
				(nClickButton==1 && theApp._configFile._nAllowMC==3) 
				|| 
				(nClickButton==2 && theApp._configFile._nAllowRC==3))
			{
				//save html
				tcf->m_pView->PostMessage(WM_COMMAND, ID_FILE_SAVEHTML, 0);
			}
			else if((nClickButton==0 && theApp._configFile._nAllowDBC==4) 
				|| 
				(nClickButton==1 && theApp._configFile._nAllowMC==4) 
				|| 
				(nClickButton==2 && theApp._configFile._nAllowRC==4))
			{
				//save html
				tcf->m_pView->PostMessage(WM_COMMAND, ID_FILE_AUTOSAVE, 0);
			}
			else if((nClickButton==0 && theApp._configFile._nAllowDBC==5) 
				|| 
				(nClickButton==1 && theApp._configFile._nAllowMC==5) 
				|| 
				(nClickButton==2 && theApp._configFile._nAllowRC==5))
			{
				tcf->bLock =!tcf->bLock;

				if(tcf->m_pView->m_nProgress<0)
				{
					int nID = ( theApp._globalSpace._pMainFrame )->FindTab(tcf);
					if(tcf->bLock)
						tcf->m_pView->oldIcon = ( theApp._globalSpace._pMainFrame )->SetTabIcon( 4, nID );
					else
						tcf->m_pView->oldIcon = ( theApp._globalSpace._pMainFrame )->SetTabIcon( -1, nID );
...
void CAdvTabCtrl::OnRButtonUp(UINT nFlags, CPoint point) 
{
	...
	if( theApp._configFile._nAllowRC > 0 && (nFlags & MK_CONTROL)!=0 )
	{
...
//  if (GetItemCount() == 0) return; // do nothing
	if( ( theApp._globalSpace._pMainFrame )->nWin == 0) return;
