// iTreeSurfView.cpp : implementation of the CiTreeSurfView class
//
...
IMPLEMENT_DYNCREATE(CiTreeSurfView, CFixedHtmlView)

BEGIN_MESSAGE_MAP(CiTreeSurfView, CFixedHtmlView)
...
////////////////////////////////////////////////////////////////////////////
// CiTreeSurfView construction/destruction

CiTreeSurfView::CiTreeSurfView()
{
	...
	m_nFontSize = theApp._configFile._nDefFontSize; //4, smallest, 3, small, 2, medium, 1, large, 0, largest
	...
	if( theApp._configFile._bNoJava )
		dwProperty |= DLCTL_NO_JAVA;

	if( theApp._configFile._bNoActivex )
		dwProperty |= DLCTL_NO_RUNACTIVEXCTLS;

	if( theApp._configFile._bNoScript )
...
//		dwProperty |= DLCTL_SILENT;
	...
	m_bAutoFilter = theApp._configFile._bEnableAutoFilter;
	...
	if(  ( theApp._globalSpace._pMainFrame )!=NULL)
		m_nRefreshInterval = theApp._configFile._nDefRefreshInterval;
}
...
BOOL CiTreeSurfView::PreCreateWindow(CREATESTRUCT& cs)
{
...
////////////////////////////////////////////////////////////////////////////
// CiTreeSurfView drawing


void CiTreeSurfView::OnInitialUpdate(int nOpenType, LPCSTR strUrl)
{
	try{
	m_bFirst = TRUE;
	...
	m_strWebProxy = theApp._configFile._csDefaultWebProxy;
...
////////////////////////////////////////////////////////////////////////////
// CiTreeSurfView printing


////////////////////////////////////////////////////////////////////////////
// CiTreeSurfView diagnostics

#ifdef _DEBUG
void CiTreeSurfView::AssertValid() const
{
	CFixedHtmlView::AssertValid();
}

void CiTreeSurfView::Dump(CDumpContext& dc) const
{
	CFixedHtmlView::Dump(dc);
}

/*CiTreeSurfDoc* CiTreeSurfView::GetDocument() // non-debug version is inline
{
//	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CiTreeSurfDoc)));
//	return (CiTreeSurfDoc*)m_pDocument;
	return NULL;
}*/
#endif //_DEBUG

////////////////////////////////////////////////////////////////////////////
// CiTreeSurfView message handlers
void CiTreeSurfView::OnTitleChange(LPCTSTR lpszText)
{
...
/*2.3
	...
	{
		if( theApp._configFile._cslPopup.Find(url)!=NULL || PopFilterStar(url))
		{
			IsPopUp = TRUE;
...
//2.3		IsPopUp=FALSE;
	...
	if( ( theApp._globalSpace._pMainFrame )!=NULL)
	{
		...
		if(tcf->m_strStickName=="")
		{
			int nTabID =  ( theApp._globalSpace._pMainFrame )->FindTab(tcf);
			if(strcmp(lpszText, "about:blank")==0)
			{
				CString tab; tab.LoadString(IDS_UNTITLED);
				 ( theApp._globalSpace._pMainFrame )->SetTabTitle( tab, nTabID );
			}
			else
				 ( theApp._globalSpace._pMainFrame )->SetTabTitle( (char*)lpszText, nTabID );
		}
		if(bSetAddress && IsActive)
		{
			 ( theApp._globalSpace._pMainFrame )->SetAddress(url);
		
		}

		//update tab bar
		if(! theApp._configFile._bAutoTab )
			 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_UPDATE_TAB);
	}
...
void WBShowScrollBar(CiTreeSurfView* pvw)
{
	try{
		...
		if( pDisp )
		{
			if (SUCCEEDED(pDisp->QueryInterface(IID_IHTMLDocument2, (void**)&pHTMLDoc)))
			{
				if(SUCCEEDED(pHTMLDoc->get_body( &pHTMLElem )))
				{
					...
					{
						pHTMLBody->put_scroll( theApp._globalSpace._bsYes );
...
void CiTreeSurfView::OnDocumentComplete(LPCTSTR lpszUrl)
{
...
//		CString strUrl;
		...
		//filter popup
		if(!m_bToClose && !IsPopUp &&  ( theApp._globalSpace._pMainFrame )!=NULL && theApp._configFile._bEnablePopupFilter && !strUrl.IsEmpty()) 
		{
	//2.2 m_IsNewPopup && 
			if(( theApp._configFile._cslPopup.Find(strUrl)!=NULL || PopFilterStar(strUrl)))
			{
				IsPopUp = TRUE;
	...
	if(glpDisp == NULL) //complete
	{
		...
		if(IsActive &&  ( theApp._globalSpace._pMainFrame )!=NULL && !strUrl.IsEmpty())
		{
			//((CMainFrame*)((CChildFrame*)GetParentFrame()->GetParentFrame()))->SetAddress(strUrl);
			 ( theApp._globalSpace._pMainFrame )->SetAddress(strUrl);
		...
		if( ( theApp._globalSpace._pMainFrame )!=NULL)
		{
			...
			if((m_nProgress>95 || m_nProgress<0))//3.2 && !strUrl.IsEmpty()
			{
				...
				{
					if(tcf->bLock)
						oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon( 4, nTabID );
					else if(tcf->bProtect)
						oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon( 6, nTabID );
					else
						oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon( -1, nTabID );

					//start to scroll
					if(!m_nScrollTimer && (m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll) && m_nSrSpeed!=0)
					{
						if(m_nProgress==100 || m_nProgress<0 || (!m_bScrollAfterDL && m_nProgress>=0))
						{
	//						m_nScrollTimer = SetTimer(2132, (UINT)(1000* theApp._configFile._fgSeed /m_nSrSpeed), NULL);
							if( theApp._configFile._bMouseCtrlScrSpeed )
								m_nLastScrSpd = STOP_TIMER;
							else
								//3.6.1 add (UINT)
								m_nLastScrSpd = (UINT)( (26 +  theApp._configFile._fgSeed  * 100)/m_nSrSpeed );
					...
					oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon( 5, nTabID );
				//update tab bar tab0
		...
		//restore homepage
		if( theApp._configFile._bLockHomepage )
		{
			 ( theApp._globalSpace._pMainFrame )->GetHomePage( ( theApp._globalSpace._pMainFrame )->m_strCurHome);
			if(strcmp( ( theApp._globalSpace._pMainFrame )->m_strHome,  ( theApp._globalSpace._pMainFrame )->m_strCurHome)!=0)
				 ( theApp._globalSpace._pMainFrame )->SetHomePage( ( theApp._globalSpace._pMainFrame )->m_strHome);
		}
	}
	...
	try{
		...
		if( theApp._configFile._bAutoSetBG )
		{
			VARIANT color;
			color.vt = VT_BSTR;
			color.bstrVal = theApp._configFile._csBGColor.AllocSysString();

			 ( theApp._globalSpace._pMainFrame )->SetBackground(this, color);
...
void CiTreeSurfView::OnBeforeNavigate2(LPCTSTR lpszURL, DWORD /*nFlags*/,
	...
	LPCTSTR /*lpszHeaders*/, BOOL* pbCancel)
{
	...
	if( theApp._configFile._bLinkInNewWindow || bLock)
	{
		if(m_tLastClick.LowPart != 0 && !baPostedData)//2.5.GetSize()==0
		{
			...
			if(liFreq.LowPart!=0)
			{
				...
				if(secs<0.6 && secs>0)
				{
					int dup = ( theApp._globalSpace._pMainFrame )->FindDupURL(lpszURL, tcf);
					if(dup<0)
						bCanNew = TRUE;
					else
					{
						*pbCancel = TRUE;
						if(mf!=NULL && theApp._configFile._bActiveNewWindow )
						{
							...
							{
								mf->MDIActivate(tcf2);
								//( theApp._globalSpace._pMainFrame )->m_wndTab.SetCurSel(m_nDupID);
							}
						}
					}
				}
		...
		if(!m_bFirst &&  ( theApp._globalSpace._pMainFrame )!=NULL && bCanNew)
		{
			...
			if(strstr(lpszURL, "mailto:")==NULL && strstr(lpszURL, "javascript:")==NULL && pdest == NULL )
			{
				...
				if(pApp==NULL)
					return;
				pWnd = ( theApp._globalSpace._pMainFrame )->NewChildWindow(0,1, NULL, this);
				if(pWnd != NULL)
				{
	...
	//filter popup
	if( ( theApp._globalSpace._pMainFrame )!=NULL && theApp._configFile._bEnablePopupFilter && lpszURL!=NULL && strcmp(lpszURL, "about:blank")!=0)
	{
//2.2 m_IsNewPopup && 
		if(( theApp._configFile._cslPopup.Find(lpszURL)!=NULL || PopFilterStar(lpszURL)))
		{
			if(pbCancel!=NULL)
				*pbCancel = TRUE;
			IsPopUp = TRUE;
		...
		else
		{
			...
			if(IsActive)
			{	
				theApp._globalSpace._iPercent = m_nProgress;
			}
		}
	...
	if( theApp._configFile._bEnableDupFilter && m_nFilterDup && !IsPopUp && m_IsNewPopup &&  ( theApp._globalSpace._pMainFrame )!=NULL && lpszURL!=NULL && strcmp(lpszURL, "about:blank")!=0)
	{
		...
		if(dup>=0)
		{
			...
			if(m_bReUsed && tcf!=NULL && mf!=NULL && ( theApp._configFile._bActiveNewWindow || tcf->m_bForceActive))
			{
			//	TCITEM TabCtrlItem;
				TabCtrlItem.mask = TCIF_PARAM;

				 ( theApp._globalSpace._pMainFrame )->m_wndTab.GetItem(m_nDupID, &TabCtrlItem);
				CChildFrame* tcf2 = (CChildFrame*)TabCtrlItem.lParam;
				if(tcf2!=NULL && tcf2!=tcf)
				{
					 ( theApp._globalSpace._pMainFrame )->MDIActivate(tcf2);
					// ( theApp._globalSpace._pMainFrame )->m_wndTab.SetCurSel(m_nDupID);
				}
	...
	if(IsActive &&  ( theApp._globalSpace._pMainFrame )!=NULL)
		 ( theApp._globalSpace._pMainFrame )->m_bIsSecure = m_bIsSecure;
...
/*		CRect rect, oldrect;
	...
	//reused window
	if(!IsPopUp && m_bReUsed && tcf!=NULL && mf!=NULL && ( theApp._configFile._bActiveNewWindow || tcf->m_bForceActive))
	{
	...
	if(!IsPopUp && bDelayAdd && tcf!=NULL && mf!=NULL)
	{
		...
		int nTabID = mf->FindTab(tcf);
		//active window
		if( theApp._configFile._bActiveNewWindow || tcf->m_bForceActive)
		{
			mf->MDIActivate(tcf);
			//select current tab
			mf->m_wndTab.SetCurSel( nTabID );
...
void CiTreeSurfView::OnGoBack()
{
	if(m_back)
		GoBack();
}

void CiTreeSurfView::OnGoForward()
{
	if(m_forward)
		GoForward();
}

void CiTreeSurfView::OnGoStartPage()
{
	GoHome();
}
...
/*void CiTreeSurfView::OnViewFontsLargest()
{
	theApp._configFile._ = m_nFontSize = 0;
...
void CiTreeSurfView::OnViewFontsLarge()
{
	theApp._configFile._ = m_nFontSize = 1;
...
void CiTreeSurfView::OnViewFontsMedium()
{
	theApp._configFile._ = m_nFontSize = 2;
...
void CiTreeSurfView::OnViewFontsSmall()
{
	theApp._configFile._ = m_nFontSize = 3;
...
void CiTreeSurfView::OnViewFontsSmallest()
{
	theApp._configFile._ = m_nFontSize = 4;
...
void CiTreeSurfView::OnNewWindow2(LPDISPATCH* ppDisp, BOOL* Cancel) 
{
	...
	*ppDisp = NULL;
	...
	if(m_bAutoFilter)
	{
		...
		//3.2
		theApp._configFile.Set_InAutoFilter( 1 );
			...
			if(!(m_bLClick && ((secs<=0.7 && secs>=0) || (bstr!=NULL && wcscmp(bstr, L"A")==0) || /*2.5*/m_nProgress==-1)) && !((Rsecs>=0 && Rsecs<=5 && m_bRClick) ||/*2.5*/ m_nProgress==-1))
			{
				*ppDisp = NULL;
				...
				if(bstr!=NULL)
					SysFreeString(bstr);
				//3.2
				theApp._configFile.Set_InAutoFilter( 0 );
				return;
			}
			if(bstr!=NULL)
			{
...
void CiTreeSurfView::OnProgressChange(long nProgress, long nProgressMax) 
{
	// TODO: Add your specialized code here and/or call the base class
	try{
...
//			m_lpszUrl = strUrl2;
	...
	if(IsActive && (m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll) )
	{	

		//start to scroll
		if(m_nScrollTimer==0 && (m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll) && m_nSrSpeed!=0)
		{
			if(m_nProgress==100 || m_nProgress<0 || (!m_bScrollAfterDL && m_nProgress>=0))
			{
//				m_nScrollTimer = SetTimer(2132, (UINT)(1000* theApp._configFile._fgSeed /m_nSrSpeed), NULL);
	...
	if( ( theApp._globalSpace._pMainFrame )!=NULL)
	{
		...
		{
			int nTabID;
			nTabID =  ( theApp._globalSpace._pMainFrame )->FindTab((CChildFrame*)GetParentFrame());
			if(m_nProgress>=0 && nIcon != oi)
				oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon( nIcon, nTabID );
			...
			//update tab bar
			if(m_nProgress*oldp<=0 && oi*oldIcon<=0 && oldIcon != oi && theApp._configFile._bFBTab )
				 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_UPDATE_TAB);
		}
	}
...
void CiTreeSurfView::OnEditCopy() 
{
...
void CiTreeSurfView::OnEditFind() 
{
	// TODO: Add your command handler code here
	 ( theApp._globalSpace._pMainFrame )->ExecCmdTarget(HTMLID_FIND, this);
}

void CiTreeSurfView::OnEditPaste() 
{
	// TODO: Add your command handler code here
...
void CiTreeSurfView::OnEditSelectAll() 
{
	// TODO: Add your command handler code here
...
void CiTreeSurfView::OnStatusTextChange(LPCTSTR lpszText) 
{
	// TODO: Add your specialized code here and/or call the base class
	try
	{
	if(IsActive &&  ( theApp._globalSpace._pMainFrame )!=NULL)
	{
		if( theApp._configFile._bURLOnly && lpszText!=NULL && strlen(lpszText)!=0)
		{
		...
		if( !(  ( theApp._globalSpace._pMainFrame )->m_bStatusBlank && lpszText!=NULL && strlen(lpszText)==0))
			 ( theApp._globalSpace._pMainFrame )->SetMessageText(lpszText);
		if(strlen(lpszText)==0)
			 ( theApp._globalSpace._pMainFrame )->m_bStatusBlank = TRUE;
		else
			 ( theApp._globalSpace._pMainFrame )->m_bStatusBlank = FALSE;
...
BOOL CiTreeSurfView::OnAmbientProperty(COleControlSite* pSite, DISPID dispid, VARIANT* pvar) 
{
	// TODO: Add your specialized code here and/or call the base class
...
void CiTreeSurfView::OnOptionsLoadimage() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_DLIMAGES;
...
void CiTreeSurfView::OnUpdateOptions(CCmdUI* pCmdUI) 
{
	...
	/*	if(dwProperty&DLCTL_DLIMAGES)
			...
			pCmdUI->SetCheck(0);*/
		 ( theApp._globalSpace._pMainFrame )->ToolBarSetCheck(pCmdUI, (dwProperty&DLCTL_DLIMAGES)!=0);
		/*
		if(dwProperty&DLCTL_DLIMAGES)
		{
			...
			{
				pCmdUI->SetCheck(0);
				 ( theApp._globalSpace._pMainFrame )->m_wndToolBar.GetToolBarCtrl().PressButton(pCmdUI->m_nID, TRUE);
			}
		}
		else
		{
			 ( theApp._globalSpace._pMainFrame )->m_wndToolBar.GetToolBarCtrl().PressButton(pCmdUI->m_nID, FALSE);
			pCmdUI->SetCheck(0);
		}*/
		break;
...
void CiTreeSurfView::OnOptionsAllowscripts() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_NO_SCRIPTS;
	ChangeProperty();
...
void CiTreeSurfView::OnOptionsLoadsounds() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_BGSOUNDS;
	ChangeProperty();
...
void CiTreeSurfView::OnOptionsLoadvideos() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_VIDEOS;
	ChangeProperty();
...
void CiTreeSurfView::OnOptionsUseWebp() 
{
	// TODO: Add your command handler code here
	if(m_bEnWebProxy)
		m_bEnWebProxy = FALSE;
...
void CiTreeSurfView::OnUpdateOptionsUseWebp(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	try
	{
		 ( theApp._globalSpace._pMainFrame )->ToolBarSetCheck(pCmdUI, m_bEnWebProxy);
	}
	catch(...)
	{
	}
...
void CiTreeSurfView::OnOptionsAllowactivex() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_NO_RUNACTIVEXCTLS;
	ChangeProperty();
...
void CiTreeSurfView::OnOptionsAllowjavaapplet() 
{
	// TODO: Add your command handler code here
	dwProperty ^= DLCTL_NO_JAVA;
	ChangeProperty();
...
void CiTreeSurfView::OnCommandStateChange(long nCommand, BOOL bEnable) 
{
	// TODO: Add your specialized code here and/or call the base class
	if(nCommand == 2)
		m_back = bEnable;
...
void CiTreeSurfView::OnUpdateGoBack(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	try{
...
void CiTreeSurfView::OnUpdateGoForward(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	try{
...
/*BOOL CiTreeSurfView::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
	// TODO: Add your specialized code here and/or call the base class
...
void ResizeSibling(CiTreeSurfView* pvw)
{
	try{
	...
	if( ( theApp._globalSpace._pMainFrame )!=NULL)
	{
		int n =  ( theApp._globalSpace._pMainFrame )->m_wndTab.GetItemCount();
		TCITEM TabCtrlItem;
		TabCtrlItem.mask = TCIF_PARAM;

		for(int i=n-1;i>=0;i--)
		{
			 ( theApp._globalSpace._pMainFrame )->m_wndTab.GetItem(i, &TabCtrlItem);
...
void CiTreeSurfView::OnSize(UINT nType, int cx, int cy) 
{
//	if (nType != SIZE_MINIMIZED )
	try{
		if( theApp._globalSpace._iMax==1)
		{
...
//				if(oldrect.Width()!=cx+4 && oldrect.Height()!=cy+4)
		...
		//size other views
		if( theApp._globalSpace._iMax == 3)
			ResizeSibling(this);
...
void CiTreeSurfView::OnUpdateViewFonts(CCmdUI* pCmdUI) 
{ 
	// TODO: Add your command update UI handler code here
...
BOOL CiTreeSurfView::PopFilterStar(LPCSTR lpszURL)
{
	CString str, url(lpszURL),tmp, tmp2;

	try{

	POSITION pos = theApp._configFile._cslPopup.GetHeadPosition();
//	url = lpszURL;
	...
	{
		str = theApp._configFile._cslPopup.GetAt(pos);
		if (str.Find('*')>=0 && str!="*")
		{
			//star filter
		...
		theApp._configFile._cslPopup.GetNext(pos);
	}
...
void CiTreeSurfView::ToNavigate(LPCTSTR URL, DWORD dwFlags, LPCTSTR lpszTargetFrameName, LPCTSTR lpszHeaders, LPVOID lpvPostData,  DWORD dwPostDataLen, BOOL bOnlyRefresh)
{
	try{
...
//		dwFlags = 4;
	...
	if(m_bEnWebProxy)
	{
		if(m_strWebProxy.IsEmpty() &&  ( theApp._globalSpace._pMainFrame )!=NULL)
			m_strWebProxy = theApp._configFile._csDefaultWebProxy;
...
void CiTreeSurfView::OnParentNotify(UINT message, LPARAM lParam) 
{
	...
	if((message == WM_LBUTTONDOWN || message == WM_RBUTTONDOWN) && ( theApp._configFile._bLinkInNewWindow || tcf->bLock || m_bAutoFilter) )
	{
/*		BSTR bstr=NULL;
		IHTMLElement* pElem = NULL;
		IHTMLDocument2* pHTMLDoc=NULL;
...
*/
	...
	if(message==WM_RBUTTONDOWN && theApp._configFile._bAllowMouseGestures )
	{
		SetCapture();
		m_bIsCapture = TRUE;
//		m_bRClick = FALSE;
...
void CiTreeSurfView::OnToolsAutoscrollAutoscrollingafterdownloading() 
{
	// TODO: Add your command handler code here
	m_bScrollAfterDL = !m_bScrollAfterDL;

	if( ( theApp._globalSpace._pMainFrame )!=NULL)
		theApp._configFile._bDefScrollAfterDL = m_bScrollAfterDL;

}

void CiTreeSurfView::OnUpdateToolsAutoscrollAutoscrollingafterdownloading(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
...
void CiTreeSurfView::OnUpdateToolsAutoscrollSpeed(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	if( theApp._configFile._bMouseCtrlScrSpeed )
		pCmdUI->Enable(FALSE);
...
/*void CiTreeSurfView::OnToolsAutoscrollMedium() 
{
	// TODO: Add your command handler code here
	m_nSrSpeed = 2;
	
	if( ( theApp._globalSpace._pMainFrame )!=NULL)
		theApp._configFile._nDefSrSpeed = m_nSrSpeed;

	ChangeSpeed(2);
}


void CiTreeSurfView::OnToolsAutoscrollSlow() 
{
	// TODO: Add your command handler code here
	m_nSrSpeed = 1;
	if( ( theApp._globalSpace._pMainFrame )!=NULL)
...
void CiTreeSurfView::OnToolsAutoscrollUseautoscrolling() 
{
	// TODO: Add your command handler code here
	m_bUseAutoScroll = !m_bUseAutoScroll;

	theApp._configFile._bDefUseAutoScroll = m_bUseAutoScroll;
	...
	else if(!(m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll) && m_nScrollTimer)
	{		//stop the timer;
		if(m_nScrollTimer)
			KillTimer(m_nScrollTimer);
		m_nScrollTimer = 0;
...
void CiTreeSurfView::OnUpdateToolsAutoscrollUseautoscrolling(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	try
	{
		 ( theApp._globalSpace._pMainFrame )->ToolBarSetCheck(pCmdUI, m_bUseAutoScroll);
	}
	catch(...)
	{
	}
...
void CiTreeSurfView::ChangeSpeed(int nSpeed)
{
	if(m_nScrollTimer)
	{		//stop the timer;
		KillTimer(m_nScrollTimer);
	...
	//start to scroll
	if(!m_nScrollTimer && (m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll) && nSpeed!=0)
	{
		if(m_nProgress==100 || m_nProgress<0 || (!m_bScrollAfterDL && m_nProgress>=0))
		{
//			m_nScrollTimer = SetTimer(2132, (UINT)(1000* theApp._configFile._fgSeed /nSpeed), NULL);
			if( theApp._configFile._bMouseCtrlScrSpeed )
...
void WBScroll(CiTreeSurfView* pvw)
{
	if(pvw->m_bToClose)
		return;
...
void CiTreeSurfView::OnTimer(UINT nIDEvent) 
{
	// TODO: Add your message handler code here and/or call default
	try{
	if(nIDEvent == m_nScrollTimer && (m_bUseAutoScroll ||  ( theApp._globalSpace._pMainFrame )->m_bAllScroll))
	{
		...
		if( theApp._configFile._bMouseCtrlScrSpeed )
		{
			...
			CRect rect;
			GetWindowRect(&rect);
			if(mousept.x<=rect.Width() && mousept.x>=rect.Width()- ( theApp._globalSpace._pMainFrame )->m_nScrollWidth)
			{
				if(mousept.y<=rect.Height()- ( theApp._globalSpace._pMainFrame )->m_nScrollArrowH && mousept.y>0)
				{
					int sp = 800 - mousept.y*800/(rect.Height()- ( theApp._globalSpace._pMainFrame )->m_nScrollArrowH)+1;
...
void CiTreeSurfView::ChangeProperty()
{
	try{
	IOleControl* poc;
	LPDISPATCH pDisp = GetHtmlDocument();
...
void CiTreeSurfView::OnToolsAutoref() 
{
	// TODO: Add your command handler code here
...
void CiTreeSurfView::OnUpdateToolsAutoref(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
...
void CiTreeSurfView::OnToolsReffre() 
{
	// TODO: Add your command handler code here
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		theApp._configFile._nDefRefreshInterval = m_nRefreshInterval;
		//change timer
		if(m_bUseAutoRef)
			ChangeRefresh();
	}
...
void CiTreeSurfView::ChangeRefresh()
{
	if(m_nAutoRefreshTimer)
	{		//stop the timer;
		KillTimer(m_nAutoRefreshTimer);
...
void CiTreeSurfView::OnNavigateComplete2(LPCTSTR lpszURL)
{
	//2.3
	try{
	CString strUrl;
	...
	if(!m_bToClose && ((IsPopUp && m_IsNewPopup) || (tcf!=NULL && !tcf->IsWindowVisible())) && !strUrl.IsEmpty())
	{
		...
		if(( ( theApp._globalSpace._pMainFrame )->IsWindowVisible() && tcf!=NULL && !tcf->IsWindowVisible()) || url2 == "navcancl.htm#" + m_strPossibleURL /*for IE 5*/|| strUrl == "about:NavigationCanceled" /*for IE4*/ || strUrl==m_strPossibleURL)
		{
			//	CChildFrame* tcf = (CChildFrame*)GetParentFrame();
				//IsActive = FALSE;
			if(tcf!=NULL) 
...
void CiTreeSurfView::OnToolsAutofilterCur() 
{
	// TODO: Add your command handler code here
	m_bAutoFilter = !m_bAutoFilter;
}

void CiTreeSurfView::OnUpdateToolsAutofilterCur(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	// ( theApp._globalSpace._pMainFrame )->ToolBarSetCheck(pCmdUI, m_bAutoFilter);
...
void CiTreeSurfView::OnFileSaveas() 
{
	// TODO: Add your command handler code here
...
void CiTreeSurfView::OnFileSavehtml() 
{
	// TODO: Add your command handler code here
...
void CiTreeSurfView::OnFilePrintSetup() 
{
	// TODO: Add your command handler code here
	ExecCommand(OLECMDID_PAGESETUP);
}

void CiTreeSurfView::OnFileProperty() 
{
	// TODO: Add your command handler code here
	ExecCommand(OLECMDID_PROPERTIES);
}

void CiTreeSurfView::ExecCommand(DWORD nCmdID)
{
...
void CiTreeSurfView::OnRButtonUp(UINT nFlags, CPoint point) 
{
	//debug
	//AfxMessageBox("CiTreeSurfView::OnRButtonUp");
	...
	if(m_bIsCapture)
	{
		...
		if(i>0)
		{
			...
			{
				// TODO: Add your command handler code here
				switch( theApp._configFile._nNewWindow2 )
				{
				case 0: 
					 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_FILE_NEW_BLANK, 0);
					break;
				case 1: 
					 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_FILE_NEW_S, 0);
					break;
				case 2: 
					 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_FILE_NEW_CURRENTPAGE, 0);
				}
			}
				...
				Refresh2(REFRESH_COMPLETELY);
				 ...
				 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_FILE_CLOSE_1, 0);
			else if(strcmp(m_SeqMG, "UL")==0)
				 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_TAB_PRE, 0);
			else if(strcmp(m_SeqMG, "UR")==0)
				 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_COMMAND, ID_TAB_NEXT, 0);
			else 
				i = 0;
		}
...
void CiTreeSurfView::OnMouseMove(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	try{
...
void CiTreeSurfView::PushMouseGesture(char gesture)
{
...
BOOL CiTreeSurfView::MoveDirection(CPoint &point, char *Direction)
{
...
void CiTreeSurfView::OnFileAutosave() 
{
	// TODO: Add your command handler code here
	try{
		...
		GetLocationURL(filename);
		 ...
		 ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath = theApp._configFile._csSavePath;
		if( theApp._configFile._bCateWithSite )
		{
			CString site;
			GetLocationURL(site);
			site.MakeLower();
			...
			if(l>=0)
			{
			 ...
			 ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath += site;
			 ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath += "\\";
			CreateDirectory( ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath, NULL);
		}

		if( theApp._configFile._nAutoSave == 0 
			|| 
			theApp._configFile._nAutoSave == 1 
			|| 
			theApp._configFile._nAutoSave == 4 
			|| 
			(
			 theApp._configFile._nAutoSave == 3 
			 && 
			 filename.Left(5)=="file:"
			)
		  )
		{
			keybd_event(VK_TAB, MapVirtualKey(VK_TAB, 0), 0, 0);
			if( theApp._configFile._nAutoSave==1 
				|| 
				theApp._configFile._nAutoSave==4
				)
				keybd_event('W', MapVirtualKey('W', 0), 0, 0);
			if( theApp._configFile._nAutoSave==4 )
			{
			...
			if(RegOpenKey(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Internet Explorer\\Main"), &hKey) == ERROR_SUCCESS)
			{
				RegSetValueEx(hKey, _T("Save Directory"), 0, REG_SZ, (LPBYTE)( ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath.GetBuffer(10)),  ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath.GetLength());
				 ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath.ReleaseBuffer();
			}
			SendMessage(WM_COMMAND, ID_FILE_SAVEAS, 0);
		}
		else if( theApp._configFile._nAutoSave == 2)
		{
			filename.Replace('/', '\\');
			if(filename.Right(1) == "\\")
			...
			filename =  ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath + filename;
			l = 0;
			...
			else
			{
				...
				if (!GetUrlCacheEntryInfo(filename,NULL,&dwEntrySize))
				{
				   ...
				   {
					   //local image files
					     ( theApp._globalSpace._pMainFrame )->DecodeEscap(filename);
						 ( theApp._globalSpace._pMainFrame )->DecodeEscap(htmlname);
		...
		else if( theApp._configFile._nAutoSave == 3)
		{
			 ( theApp._globalSpace._pMainFrame )->m_bIsAutoSave = TRUE;
			 ( theApp._globalSpace._pMainFrame )->m_pCurView = this;
			 ( theApp._globalSpace._pMainFrame )->SendMessage(WM_COMMAND, ID_FILE_QUICKSAVE,0);
			 ( theApp._globalSpace._pMainFrame )->m_bIsAutoSave = FALSE;
		}

		l =  ( theApp._globalSpace._pMainFrame )->FindTab((CChildFrame*)GetParentFrame());
		oldIcon =  ( theApp._globalSpace._pMainFrame )->SetTabIcon(7, l);
		if( theApp._configFile._bFBTab )
			 ( theApp._globalSpace._pMainFrame )->PostMessage(WM_UPDATE_TAB);
		CString savedmsg;
		savedmsg.LoadString(IDS_PAGE_SAVED);
		savedmsg+=  ( theApp._globalSpace._pMainFrame )->m_strAutoSavePath;
		 ( theApp._globalSpace._pMainFrame )->SetMessageText(savedmsg);
