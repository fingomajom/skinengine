// iTreeSurfNSHandle.cpp : Implementation of CiTreeSurfApp and DLL registration.
...
CiTreeSurfNSHandle::CiTreeSurfNSHandle()
{
}


STDMETHODIMP CiTreeSurfNSHandle::Start(
        LPCWSTR szUrl,
        ...
        DWORD dwReserved)
{
	...
	if( theApp._configFile._bUseUrlFilter && ! ( theApp._globalSpace._pMainFrame )->m_bUrlFilterDirty)
	{
		CString strUrl(szUrl); void* tmp;
		strUrl.MakeLower();
		BOOL bFilter = FALSE;
		if( theApp._globalSpace._mapUrlFilter.Lookup(strUrl, tmp))
			bFilter = TRUE;
...
STDMETHODIMP CiTreeSurfNSHandle::Continue(PROTOCOLDATA *pStateInfo)
{
	return S_OK;
}

STDMETHODIMP CiTreeSurfNSHandle::Abort(HRESULT hrReason,DWORD dwOptions)
{
	return E_NOTIMPL;
}

STDMETHODIMP CiTreeSurfNSHandle::Terminate(DWORD dwOptions)
{
	return  S_OK;
}

STDMETHODIMP CiTreeSurfNSHandle::Suspend()
{
...
STDMETHODIMP CiTreeSurfNSHandle::Resume()
{
...
STDMETHODIMP CiTreeSurfNSHandle::Read(void *pv, ULONG cb, ULONG *pcbRead)
{
	try{
...
STDMETHODIMP CiTreeSurfNSHandle::Seek(
...
STDMETHODIMP CiTreeSurfNSHandle::LockRequest(DWORD dwOptions)
{
...
STDMETHODIMP CiTreeSurfNSHandle::UnlockRequest()
{
...
STDMETHODIMP CiTreeSurfNSHandle::CombineUrl(LPCWSTR pwzBaseUrl, LPCWSTR pwzRelativeUrl, DWORD dwCombineFlags,
...
STDMETHODIMP CiTreeSurfNSHandle::CompareUrl(LPCWSTR pwzUrl1, LPCWSTR pwzUrl2, DWORD dwCompareFlags)
{
...
STDMETHODIMP CiTreeSurfNSHandle::ParseUrl(LPCWSTR pwzUrl, PARSEACTION ParseAction, DWORD dwParseFlags,
	LPWSTR pwzResult, DWORD cchResult, DWORD *pcchResult, DWORD dwReserved)
{
...
STDMETHODIMP CiTreeSurfNSHandle::QueryInfo( LPCWSTR pwzUrl, QUERYOPTION QueryOption, DWORD dwQueryFlags,
...
inline BOOL CiTreeSurfNSHandle::PatternFilter(CString & url)
{
	...
	BOOL final = FALSE;
	int p = 0;
	while(p<= theApp._globalSpace._csaUrlPattern.GetUpperBound() && !isame)
	{
		str = theApp._globalSpace._csaUrlPattern.GetAt(p);
		//star filter
