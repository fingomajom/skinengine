...
BOOL CQuickSearchDlg::OnInitDialog() 
{
	...
	POSITION pos = theApp._configFile._mapQuickSearch.GetStartPosition();
	CString name, url;
	while(pos!=NULL)
	{
		theApp._configFile._mapQuickSearch.GetNextAssoc(pos, name, url);
...
void CQuickSearchDlg::OnOK() 
{
	...
	theApp._configFile.Set_QuickSearch( m_EngineList );
