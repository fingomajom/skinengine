...
BOOL CPopupDlg::OnInitDialog() 
{
	...
	while(pos!=NULL)
	{
		m_PopupList.InsertItem(i, theApp._configFile._cslPopup.GetAt(pos));
...
void CPopupDlg::OnOK() 
{
	...
	theApp._configFile._cslPopup.RemoveAll();
	int n = m_PopupList.GetItemCount();
	for( int i=0; i<n; i++)
	{
		theApp._configFile._cslPopup.AddTail(m_PopupList.GetItemText(i,0));
	}
