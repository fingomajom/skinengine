...
public:
	...
	STDMETHOD(CreateControl)( HWND hWnd, REFCLSID rclsid )
	{
		CComPtr<IUnknown> p;
		...
		HRESULT hr = CoCreateInstance(rclsid, NULL, CLSCTX_INPROC_SERVER, __uuidof(IUnknown), (void**)&m_spUnknown);
		for (int i = 1; i < 10; ++i)
		{
			if (FAILED(hr) || !m_spUnknown)
				hr = CoCreateInstance(rclsid, NULL, CLSCTX_INPROC_SERVER, __uuidof(IUnknown), (void**)&m_spUnknown);
			else
				break ;
